...
__name(rpad_, "rpad_");
function pad0r1(v, d2) {
  var t = "" + Math.round(v);
  return t.length >= d2 ? t : fill("0", d2 - t.length) + t;
}
__name(pad0r1, "pad0r1");
function pad0r2(v, d2) {
  var t = "" + v;
  return t.length >= d2 ? t : fill("0", d2 - t.length) + t;
}
...
function pad0r(v, d2) {
  if (v > p2_32 || v < -p2_32) return pad0r1(v, d2);
  var i2 = Math.round(v);
  return pad0r2(i2, d2);
}
__name(pad0r, "pad0r");
...
function SSF_parse_date_code(v, opts, b2) {
  ...
  var date = v | 0, time3 = Math.floor(86400 * (v - date)), dow = 0;
  var dout = [];
  var out = { D: date, T: time3, u: 86400 * (v - date) - time3, y: 0, m: 0, d: 0, H: 0, M: 0, S: 0, q: 0 };
...
function eval_fmt(fmt, v, opts, flen) {
  var out = [], o2 = "", i2 = 0, c2 = "", lst = "t", dt, j, cc;
  var hr = "H";
  while (i2 < fmt.length) {
    switch (c2 = fmt.charAt(i2)) {
      case "G":
...
function dateNF_fix(str, dateNF, match) {
  ...
  var datestr = ("" + (Y >= 0 ? Y : (/* @__PURE__ */ new Date()).getFullYear())).slice(-4) + "-" + ("00" + (m >= 1 ? m : 1)).slice(-2) + "-" + ("00" + (d2 >= 1 ? d2 : 1)).slice(-2);
  if (datestr.length == 7) datestr = "0" + datestr;
  if (datestr.length == 8) datestr = "20" + datestr;
  var timestr = ("00" + (H >= 0 ? H : 0)).slice(-2) + ":" + ("00" + (M >= 0 ? M : 0)).slice(-2) + ":" + ("00" + (S >= 0 ? S : 0)).slice(-2);
  if (H == -1 && M == -1 && S == -1) return datestr;
  if (Y == -1 && m == -1 && d2 == -1) return timestr;
...
var CRC32 = /* @__PURE__ */ function() {
  ...
  function slice_by_16_tables(T) {
    var c2 = 0, v = 0, n = 0, table3 = typeof Int32Array !== "undefined" ? new Int32Array(4096) : new Array(4096);
    for (n = 0; n != 256; ++n) table3[n] = T[n];
    for (n = 0; n != 256; ++n) {
      v = T[n];
      for (c2 = 256 + n; c2 < 4096; c2 += 256) v = table3[c2] = v >>> 8 ^ T[v & 255];
    }
    var out = [];
    for (n = 1; n != 16; ++n) out[n - 1] = typeof Int32Array !== "undefined" ? table3.subarray(n * 256, n * 256 + 256) : table3.slice(n * 256, n * 256 + 256);
    return out;
  }
...
var CFB = /* @__PURE__ */ (/* @__PURE__ */ __name(function _CFB() {
  ...
  function namecmp(l2, r) {
    ...
    for (var i3 = 0, c2 = 0, Z = Math.min(L.length, R.length); i3 < Z; ++i3) {
      if (c2 = L[i3].length - R[i3].length) return c2;
...
function fuzzydate(s2) {
  ...
  var y = o2.getYear(), m = o2.getMonth(), d2 = o2.getDate();
...
function utf8reada(orig) {
  var out = "", i2 = 0, c2 = 0, d2 = 0, e2 = 0, f = 0, w = 0;
  while (i2 < orig.length) {
    ...
    w = ((c2 & 7) << 18 | (d2 & 63) << 12 | (e2 & 63) << 6 | f & 63) - 65536;
...
function utf8readb(data) {
  var out = new_raw_buf(2 * data.length), w, i2, j = 1, k = 0, ww = 0, c2;
  for (i2 = 0; i2 < data.length; i2 += j) {
    j = 1;
    if ((c2 = data.charCodeAt(i2)) < 128) w = c2;
    else if (c2 < 224) {
      w = (c2 & 31) * 64 + (data.charCodeAt(i2 + 1) & 63);
      j = 2;
    } else if (c2 < 240) {
      w = (c2 & 15) * 4096 + (data.charCodeAt(i2 + 1) & 63) * 64 + (data.charCodeAt(i2 + 2) & 63);
      j = 3;
    } else {
      j = 4;
      w = (c2 & 7) * 262144 + (data.charCodeAt(i2 + 1) & 63) * 4096 + (data.charCodeAt(i2 + 2) & 63) * 64 + (data.charCodeAt(i2 + 3) & 63);
      w -= 65536;
      ww = 55296 + (w >>> 10 & 1023);
      w = 56320 + (w & 1023);
    }
...
function read_double_le(b, idx) {
  ...
  if (e2 == 2047) return m == 0 ? s2 * Infinity : NaN;
  if (e2 == 0) e2 = -1022;
  else {
    e2 -= 1023;
    m += Math.pow(2, 52);
  }
  return s2 * Math.pow(2, e2 - 52) * m;
}
__name(read_double_le, "read_double_le");
function write_double_le(b, v, idx) {
  var bs = (v < 0 || 1 / v == -Infinity ? 1 : 0) << 7, e2 = 0, m = 0;
  var av = bs ? -v : v;
  if (!isFinite(av)) {
    e2 = 2047;
    m = isNaN(v) ? 26985 : 0;
    ...
    m = av * Math.pow(2, 52 - e2);
    if (e2 <= -1023 && (!isFinite(m) || m < Math.pow(2, 52))) {
      e2 = -1022;
    } else {
      m -= Math.pow(2, 52);
      e2 += 1023;
...
var ___utf16le = /* @__PURE__ */ __name(function(b, s2, e2) {
  var ss = [];
  for (var i2 = s2; i2 < e2; i2 += 2) ss.push(String.fromCharCode(__readUInt16LE(b, i2)));
  return ss.join("").replace(chr0, "");
}, "___utf16le");
var __utf16le = has_buf ? function(b, s2, e2) {
  if (!Buffer.isBuffer(b)) return ___utf16le(b, s2, e2);
  return b.toString("utf16le", s2, e2).replace(chr0, "");
} : ___utf16le;
var ___hexlify = /* @__PURE__ */ __name(function(b, s2, l2) {
  var ss = [];
  for (var i2 = s2; i2 < s2 + l2; ++i2) ss.push(("0" + b[i2].toString(16)).slice(-2));
  return ss.join("");
}, "___hexlify");
var __hexlify = has_buf ? function(b, s2, l2) {
  return Buffer.isBuffer(b) ? b.toString("hex", s2, s2 + l2) : ___hexlify(b, s2, l2);
} : ___hexlify;
var ___utf8 = /* @__PURE__ */ __name(function(b, s2, e2) {
  var ss = [];
  for (var i2 = s2; i2 < e2; i2++) ss.push(String.fromCharCode(__readUInt8(b, i2)));
  return ss.join("");
}, "___utf8");
var __utf8 = has_buf ? /* @__PURE__ */ __name(function utf8_b(b, s2, e2) {
  return Buffer.isBuffer(b) ? b.toString("utf8", s2, e2) : ___utf8(b, s2, e2);
}, "utf8_b") : ___utf8;
var ___lpstr = /* @__PURE__ */ __name(function(b, i2) {
  var len = __readUInt32LE(b, i2);
  return len > 0 ? __utf8(b, i2 + 4, i2 + 4 + len - 1) : "";
}, "___lpstr");
var __lpstr = ___lpstr;
var ___cpstr = /* @__PURE__ */ __name(function(b, i2) {
  var len = __readUInt32LE(b, i2);
  return len > 0 ? __utf8(b, i2 + 4, i2 + 4 + len - 1) : "";
}, "___cpstr");
var __cpstr = ___cpstr;
var ___lpwstr = /* @__PURE__ */ __name(function(b, i2) {
  var len = 2 * __readUInt32LE(b, i2);
  return len > 0 ? __utf8(b, i2 + 4, i2 + 4 + len - 1) : "";
}, "___lpwstr");
var __lpwstr = ___lpwstr;
var ___lpp4 = /* @__PURE__ */ __name(function lpp4_(b, i2) {
  var len = __readUInt32LE(b, i2);
  return len > 0 ? __utf16le(b, i2 + 4, i2 + 4 + len) : "";
}, "lpp4_");
var __lpp4 = ___lpp4;
var ___8lpp4 = /* @__PURE__ */ __name(function(b, i2) {
  var len = __readUInt32LE(b, i2);
...
function write_core_props(cp, _opts) {
  ...
  for (var i2 = 0; i2 != CORE_PROPS.length; ++i2) {
    ...
    if (v != null) cp_doit(f[0], v, null, o2, p2);
  }
  if (o2.length > 2) {
...
function xlml_write_custprops(Props, Custprops) {
  ...
  if (Props) keys(Props).forEach(function(k) {
    ...
    o2.push(writextag(escapexmltag(k), m, { "dt:dt": t }));
  });
...
function write_FILETIME(time3) {
  var date = typeof time3 == "string" ? new Date(Date.parse(time3)) : time3;
...
function write_Bes(v, t, o2) {
  if (!o2) o2 = new_buf(2);
  o2.write_shift(1, t == "e" ? +v : +!!v);
...
var DIF = /* @__PURE__ */ function() {
  ...
  var sheet_to_dif = /* @__PURE__ */ function() {
    var push_field = /* @__PURE__ */ __name(function pf(o2, topic, v, n, s2) {
      o2.push(topic);
      o2.push(v + "," + n);
      o2.push('"' + s2.replace(/"/g, '""') + '"');
    }, "pf");
    var push_value = /* @__PURE__ */ __name(function po(o2, type, v, s2) {
      o2.push(type + "," + v);
...
var WK_ = /* @__PURE__ */ function() {
  function lotushopper(data, cb, opts) {
    ...
    while (data.l < data.length) {
      ...
      var d2 = R.f && R.f(data, length, opts);
      data.l = tgt;
      if (cb(d2, R, RT)) return;
    }
  }
...
var RTF = /* @__PURE__ */ function() {
  ...
  function sheet_to_rtf(ws) {
    ...
    for (var R = r.s.r; R <= r.e.r; ++R) {
      ...
      for (var C = r.s.c; C <= r.e.c; ++C) o2.push("\\cellx" + (C + 1));
      o2.push("\\pard\\intbl");
...
function write_comments_xml(data) {
  ...
  data.forEach(function(d2) {
    ...
    if (ts.length <= 1) o2.push(writetag("t", escapexml(ts[0] || "")));
      ...
      var t = "Comment:\n    " + ts[0] + "\n";
      for (var i2 = 1; i2 < ts.length; ++i2) t += "Reply:\n    " + ts[i2] + "\n";
...
function parse_PtgExtraMem(blob, cce, opts) {
  ...
  var out = [];
  for (var i2 = 0; i2 != count3; ++i2) out.push((opts.biff == 12 ? parse_UncheckedRfX : parse_Ref8U)(blob, 8));
  return out;
}
...
function write_ws_xml_autofilter(data, ws, wb, idx) {
  ...
  if (i2 == names.length) names.push({ Name: "_xlnm._FilterDatabase", Sheet: idx, Ref: "'" + wb.SheetNames[idx] + "'!" + ref2 });
...
function write_ws_xml_cell(cell, ref2, ws, opts) {
  ...
  var v = writetag("v", escapexml(vv)), o2 = { r: ref2 };
...
function write_ws_xml_data(ws, opts, idx, wb) {
  var o2 = [], r = [], range = safe_decode_range(ws["!ref"]), cell = "", ref2, rr = "", cols = [], R = 0, C = 0, rows = ws["!rows"];
  var dense = Array.isArray(ws);
  ...
  for (R = range.s.r; R <= range.e.r; ++R) {
    ...
    for (C = range.s.c; C <= range.e.c; ++C) {
      ...
      if ((cell = write_ws_xml_cell(_cell, ref2, ws, opts, idx, wb)) != null) r.push(cell);
    }
...
function write_ws_xml(idx, opts, wb, rels) {
  ...
  if (!opts || opts.ignoreEC || opts.ignoreEC == void 0) o2[o2.length] = writetag("ignoredErrors", writextag("ignoredError", null, { numberStoredAsText: 1, sqref: ref2 }));
...
function write_ws_xlml(idx, opts, wb) {
  ...
  if (t.length > 0) o2.push("<Names>" + t + "</Names>");
  t = ws ? write_ws_xlml_table(ws, opts, idx, wb) : "";
  if (t.length > 0) o2.push("<Table>" + t + "</Table>");
...
function write_ws_biff8(idx, opts, wb) {
  ...
  for (var R = range.s.r; R <= range.e.r; ++R) {
    ...
    for (var C = range.s.c; C <= range.e.c; ++C) {
      ...
      if (b8 && cell.l) ws["!links"].push([ref2, cell.l]);
    }
  }
...
function make_html_row(ws, r, R, o2) {
  var M = ws["!merges"] || [];
  var oo = [];
  for (var C = r.s.c; C <= r.e.c; ++C) {
    var RS = 0, CS = 0;
...
function sheet_to_html(ws, opts) {
  ...
  for (var R = r.s.r; R <= r.e.r; ++R) out.push(make_html_row(ws, r, R, o2));
  out.push("</table>" + footer);
  return out.join("");
}
__name(sheet_to_html, "sheet_to_html");
function sheet_add_dom(ws, table3, _opts) {
  var opts = _opts || {};
...
function parse_dom_table(table3, _opts) {
  var opts = _opts || {};
  var ws = opts.dense ? [] : {};
  return sheet_add_dom(ws, table3, _opts);
}
__name(parse_dom_table, "parse_dom_table");
function table_to_book(table3, opts) {
  return sheet_to_workbook(parse_dom_table(table3, opts), opts);
}
...
var write_content_ods = /* @__PURE__ */ function() {
  ...
  return /* @__PURE__ */ __name(function wcx(wb, opts) {
    ...
    for (var i2 = 0; i2 != wb.SheetNames.length; ++i2) o2.push(write_ws2(wb.Sheets[wb.SheetNames[i2]], wb, i2, opts));
    o2.push("    </office:spreadsheet>\n");
    o2.push("  </office:body>\n");
...
function make_json_row(sheet, r, R, cols, header, hdr, dense, o2) {
  var rr = encode_row(R);
...
function sheet_to_json(sheet, opts) {
  ...
  for (R = r.s.r + offset; R <= r.e.r; ++R) {
    ...
    var row = make_json_row(sheet, r, R, cols, header, hdr, dense, o2);
...
var qreg = /"/g;
function make_csv_row(sheet, r, R, cols, fs, rs, FS, o2) {
  var isempty = true;
...
function sheet_to_csv(sheet, opts) {
  ...
  for (var R = r.s.r; R <= r.e.r; ++R) {
    if ((rowinfo[R] || {}).hidden) continue;
    row = make_csv_row(sheet, r, R, cols, fs, rs, FS, o2);
    if (row == null) {
      continue;
    }
...
// src/index.ts
var router = e({ base: "/api" });
var STORAGE_BASE_URL = "https://storage.youware.me";
var corsHeaders = {
...
var applyCors = /* @__PURE__ */ __name((response, origin) => {
...
var json = /* @__PURE__ */ __name((data, message = "ok", status = 200, code = 0, origin) => applyCors(
  new Response(
    JSON.stringify({
      code,
      message,
...
var jsonError = /* @__PURE__ */ __name((message, status = 400, code = status, origin) => json(null, message, status, code, origin), "jsonError");
var withTiming = /* @__PURE__ */ __name((handler) => {
  return async (request, env2, ctx) => {
    const start = Date.now();
    try {
      const response = await handler(request, env2, ctx);
      if (response) {
...
var fetchJson = /* @__PURE__ */ __name(async (input, init) => {
...
var requestDownloadPresign = /* @__PURE__ */ __name(async (keys2) => {
  const unique = Array.from(new Set(keys2.filter(Boolean)));
  ...
  try {
    if (unique.length === 1) {
      ...
      const payload = await fetchJson(`${STORAGE_BASE_URL}/presign/download/batch`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    console.error("Failed to request download presign", error3);
  }
  return map;
}, "requestDownloadPresign");
var isHostedStorageKey = /* @__PURE__ */ __name((value) => {
...
var resolveHostedIconUrl = /* @__PURE__ */ __name((rawValue, map) => {
...
var parseJson = /* @__PURE__ */ __name(async (request) => {
  try {
    ...
    console.error("Failed to parse JSON body", error3);
    return null;
  }
}, "parseJson");
var slugify = /* @__PURE__ */ __name((input) => input.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 64), "slugify");
var sanitizeTag = /* @__PURE__ */ __name((tag) => tag.trim().toLowerCase().replace(/\s+/g, "-").slice(0, 48), "sanitizeTag");
var requireAuth = /* @__PURE__ */ __name((request) => {
...
var ensureAuthenticated = /* @__PURE__ */ __name((ctx) => {
...
var fetchCategoryById = /* @__PURE__ */ __name(async (env2, id) => {
  const statement = env2.DB.prepare(
    `SELECT c.*, (
...
var fetchWordById = /* @__PURE__ */ __name(async (env2, id) => {
  const statement = env2.DB.prepare(
    `SELECT
        w.id,
        w.word,
  ...
  const record = await statement.bind(id).first();
  if (!record) return void 0;
  const [mapped] = await mapWordRows([record]);
  return mapped;
}, "fetchWordById");
var numberFromParam = /* @__PURE__ */ __name((value) => {
...
router.get(
  "/health",
  withTiming(
    async () => json({ timestamp: (/* @__PURE__ */ new Date()).toISOString() }, "ok")
  )
);
router.get(
  "/categories",
  withTiming(async (request, env2) => {
...
router.get(
  ...
  withTiming(async (request, env2) => {
    ...
    const category = await fetchCategoryById(env2, id);
...
router.post(
  ...
  withTiming(async (request, env2) => {
    ...
    try {
      const insert = await env2.DB.prepare(
      ...
      const category = await fetchCategoryById(env2, Number(categoryId));
      ...
      console.error("Category creation failed", error3);
      return jsonError("Failed to create category", 500);
    }
  })
);
router.put(
  ...
  withTiming(async (request, env2) => {
    ...
    try {
      ...
      console.error("Category update failed", error3);
...
router.delete(
  ...
  withTiming(async (request, env2) => {
    ...
    const id = numberFromParam(request.params?.id ?? null);
    if (!id) return jsonError("Invalid category id", 400);
    const result = await env2.DB.prepare("DELETE FROM categories WHERE id = ?").bind(id).run();
    if (result.meta.changes === 0) {
      return jsonError("Category not found", 404);
    }
...
router.get(
  ...
  withTiming(async (request, env2) => {
    ...
    const itemsStatement = env2.DB.prepare(itemsQuery).bind(...params, pageSize, offset);
    const countStatement = env2.DB.prepare(countQuery).bind(...params);
    const [{ results }, totalRow] = await Promise.all([
      itemsStatement.all(),
      countStatement.first()
    ]);
    const mappedRows = await mapWordRows(results);
...
router.get(
  ...
  withTiming(async (request, env2) => {
    ...
    const word = await fetchWordById(env2, id);
...
router.post(
  "/export",
  withTiming(async (request) => {
    try {
      const body = await parseJson(request);
      if (!body || !Array.isArray(body.rows) || body.rows.length === 0) {
        const origin2 = request.headers.get("Origin") || void 0;
        return jsonError("rows array is required", 422, 422, origin2);
      }
      const headers = [
        "Word",
        "Arabic Translation",
      ...
      const keysNeedingPresign = body.rows.flatMap((row) => {
        ...
        if (wordIconKey) keys2.push(wordIconKey);
        if (categoryIconKey) keys2.push(categoryIconKey);
        return keys2;
      });
      const downloadMap = await requestDownloadPresign(keysNeedingPresign);
      const fetchImageAsBase64 = /* @__PURE__ */ __name(async (url) => {
        try {
          ...
          try {
            const base64 = btoa(String.fromCharCode(...new Uint8Array(arrayBuffer2)));
          ...
          console.error("Failed to fetch image:", url, error3);
          return null;
        }
      }, "fetchImageAsBase64");
      const rows = [];
      const imageRefs = {};
      for (let i2 = 0; i2 < body.rows.length; i2++) {
        const row = body.rows[i2];
        const rowIndex = i2 + 2;
        const resolveIcon = /* @__PURE__ */ __name(async (value, cell) => {
          const trimmed = value?.trim() ?? "";
          if (!trimmed) return "";
        ...
        const truncateText = /* @__PURE__ */ __name((text, maxLength = 32767) => {
          if (!text) return "";
          return text.length > maxLength ? text.substring(0, maxLength) : text;
        }, "truncateText");
        rows.push([
          truncateText(row.word),
          truncateText(row.arabicTranslation),
          wordIcon,
          truncateText(row.turkishSentence),
          truncateText(row.arabicSentence),
          truncateText(row.categoryName),
          categoryIcon,
          truncateText(row.difficultyLevel),
          truncateText(row.vowelHarmonyRule),
          truncateText(Array.isArray(row.tags) ? row.tags.join(", ") : "")
        ]);
      }
      ...
      utils.sheet_add_aoa(worksheet, [["Exported at", (/* @__PURE__ */ new Date()).toISOString()]], {
        origin: "K1"
      });
      const workbook = utils.book_new();
      utils.book_append_sheet(workbook, worksheet, "Vocabulary");
      const arrayBuffer = writeSync(workbook, { type: "array", bookType: "xlsx" });
      ...
      console.error("Export failed", error3);
      const origin = request.headers.get("Origin") || void 0;
      return jsonError(`Export failed: ${error3 instanceof Error ? error3.message : "Unknown error"}`, 500, 500, origin);
    }
  })
);
router.post(
  ...
  withTiming(async (request, env2) => {
    ...
    const tags = Array.isArray(body.tags) ? body.tags.map(sanitizeTag).filter(Boolean) : [];
    let wordId = null;
    try {
      const insert = await env2.DB.prepare(
        `INSERT INTO words (
      ...
      if (tags.length) {
        const statements = tags.map(
          (tag) => env2.DB.prepare("INSERT INTO word_tags (word_id, tag) VALUES (?, ?)").bind(wordId, tag)
        );
        await env2.DB.batch(statements);
      }
      const createdWord = await fetchWordById(env2, wordId);
      return json(createdWord ?? null, "word created", 201);
    } catch (error3) {
      console.error("Word creation failed", error3);
      if (wordId) {
        await env2.DB.prepare("DELETE FROM words WHERE id = ?").bind(wordId).run();
      }
      return jsonError("Failed to create word", 500);
    }
  })
...
router.put(
  ...
  withTiming(async (request, env2) => {
    ...
    if (body.arabicTranslation !== void 0) {
    ...
    if (updates.length === 0 && body.tags === void 0) {
      return jsonError("No updatable fields provided", 400);
    }
    updates.push("updated_at = CURRENT_TIMESTAMP");
    updates.push("updated_by = ?");
    values.push(user.userId);
    const updateResult = await env2.DB.prepare(
    ...
    if (body.tags !== void 0) {
      const tags = Array.isArray(body.tags) ? body.tags.map(sanitizeTag).filter(Boolean) : [];
      const statements = [env2.DB.prepare("DELETE FROM word_tags WHERE word_id = ?").bind(id)];
      tags.forEach((tag) => {
        statements.push(env2.DB.prepare("INSERT INTO word_tags (word_id, tag) VALUES (?, ?)").bind(id, tag));
      });
      await env2.DB.batch(statements);
    }
    const updatedWord = await fetchWordById(env2, id);
...
router.delete(
  ...
  withTiming(async (request, env2) => {
    ...
    const id = numberFromParam(request.params?.id ?? null);
    if (!id) return jsonError("Invalid word id", 400);
    const result = await env2.DB.prepare("DELETE FROM words WHERE id = ?").bind(id).run();
    if (result.meta.changes === 0) {
      return jsonError("Word not found", 404);
    }
...
var normalizeBasePath = /* @__PURE__ */ __name((input) => {
  if (!input) return "/api";
...
var joinBaseWithPath = /* @__PURE__ */ __name((basePath, targetPath) => {
...
var src_default = {
  async fetch(request, env2, ctx) {
    const basePath = normalizeBasePath(env2.API_BASE_PATH);
    ...
    const response = await router.handle(effectiveRequest, env2, ctx);
    if (response) {
      return response;
    }
...
// .wrangler/tmp/bundle-AeoNrY/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-AeoNrY/middleware-loader.entry.ts
