...
var applyCors = /* @__PURE__ */ __name((response) => {
  Object.entries(corsHeaders).forEach(([key, value]) => {
    response.headers.set(key, value);
  });
  return response;
}, "applyCors");
var json = /* @__PURE__ */ __name((data, message = "ok", status = 200, code = 0) => applyCors(
  new Response(
    JSON.stringify({
      code,
      message,
...
var jsonError = /* @__PURE__ */ __name((message, status = 400, code = status) => json(null, message, status, code), "jsonError");
...
router.post(
  ...
  withTiming(async (request) => {
    ...
    if (body.embedImages) {
      console.warn(
        "/export called with embedImages=true, but inline embedding is not yet supported. Falling back to icon URLs only."
      );
    }
    const headers = [
      "Word",
    ...
    const rows = body.rows.map((row) => {
      const resolveIcon = /* @__PURE__ */ __name((value) => {
        const trimmed = value?.trim() ?? "";
        if (!trimmed) return "";
        if (/^https?:/i.test(trimmed) || trimmed.startsWith("data:")) {
          return trimmed;
        }
        return downloadMap.get(trimmed)?.url ?? trimmed;
      }, "resolveIcon");
      return [
        row.word ?? "",
        row.arabicTranslation ?? "",
        resolveIcon(row.wordIconKey),
        row.turkishSentence ?? "",
        row.arabicSentence ?? "",
        row.categoryName ?? "",
        resolveIcon(row.categoryIconKey),
        row.difficultyLevel ?? "",
...
router.all(
  "*",
  withTiming(() => jsonError("Not Found", 404))
);
...
// .wrangler/tmp/bundle-VIk4Hd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-VIk4Hd/middleware-loader.entry.ts
