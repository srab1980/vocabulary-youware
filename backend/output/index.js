...
var fetchWordById = /* @__PURE__ */ __name(async (env2, id) => {
  ...
  const tags = record.tagList?.split(",").filter(Boolean) ?? [];
  delete record.tagList;
  return { ...record, tags };
}, "fetchWordById");
...
router.post(
  ...
  withTiming(async (request) => {
    ...
    if (body.embedImages) {
      console.warn("/export called with embedImages=true, but inline embedding is not yet supported. Falling back to icon URLs only.");
    }
    const headers = [
      "Word",
      "Arabic Translation",
